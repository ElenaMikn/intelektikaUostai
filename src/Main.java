...
public class Main {
    ...
    public static void Skaiciavimas(Ship ship, Harbor harbor, Shipments shipments, Shipments shipmentsClone) {
         int maxWeight=ship.getMaxWeight();
        ...
        int saveSkipped = harbor.getHarborSize() * harbor.getHarborsNames().indexOf(shipments.getInitialHarbor());
        ...
        for (int harCountMultiplier = 0; harCountMultiplier <= Math.pow(Double.valueOf(harbor.getHarborSize()) - 1, 2) * 2; harCountMultiplier = harCountMultiplier + harbor.getHarborSize()) {
            ...
            int[]  tempAction = new int[harbor.getHarborSize()];
                ...
                if (showTempAction <= maxWeight) {
                    ...
                    if (tempSaveF < saveF || saveF == -1) {
                        ...
                        if (!checkRepeated) {
                            ...
                            if (size > 1) {
                                if (result.get(size - 1).res < result.get(size - 2).res && result.get(size - 1).level == result.get(size - 2).level)
    ...
    public static void main(String[] args) {
       ...
       Scanner readShipMaxWeight = new Scanner(System.in);
        ...
        //EM. man reikalingas toShipmentsClone???
        ...
        Skaiciavimas(ship, harbor, shipments, shipmentsClone);
        ...
        while (true) {
            int i = 0;
            for (i = 0; i < shipments.getListOfShipments().size(); i++) {
                if (shipments.getListOfShipments().get(i).getWeight() != 0) break;
            }
            if (i == shipments.getListOfShipments().size()) break;
